(() => {
  ...
  const goalCards=()=>{
    ...
    return goals.map(goal=>{const s=goalStats(goal,planning);return `<article class="calibration-goal"><div><strong>${escapeHtml(goal.title)}</strong><span>近7天打卡 ${s.last7}${goal.targetPerWeek?`/${goal.targetPerWeek}`:""} · 近30天关联任务专注 ${formatMinutes(s.effort30)} · 关联任务完成 ${s.done}/${s.linked.length}</span></div><div class="calibration-goal-target"><label>每周目标次数 · 可选</label><input class="text-input" type="number" min="1" max="7" data-cal-goal-target="${escapeHtml(goal.id)}" value="${goal.targetPerWeek||""}" placeholder="例如 3"><button class="mini-btn" data-cal-goal-save="${escapeHtml(goal.id)}" type="button">保存</button></div></article>`;}).join("");
  };
  const decisionCards=()=>{
  ...
  const bind=()=>{
    ...
    mount.querySelectorAll("[data-cal-goal-save]").forEach(btn=>btn.onclick=()=>{const id=btn.dataset.calGoalSave,input=mount.querySelector(`[data-cal-goal-target="${CSS.escape(id)}"]`),data=planningData(),goal=data.goals.find(g=>g.id===id);if(!goal)return;const n=Number(input?.value)||0;goal.targetPerWeek=n?Math.min(7,Math.max(1,n)):null;goal.updatedAt=now();savePlanning(data);render();toast("目标节奏已保存");});
