(() => {
  ...
  registerTool({ id:"pomodoro", icon:"◴", name:"番茄钟", category:"专注", desc:"围绕一件事进行专注循环，并积累历史。", keywords:"番茄钟 pomodoro 专注 倒计时 休息 待办" }, (root,ctx={}) => {
    ...
    const draw=()=>{sync();const t=trend();time.textContent=fmt(s.remaining);mode.textContent=`${s.mode} · 已完成 ${s.cycles} 个番茄`;taskLabel.textContent=s.currentTaskTitle?`当前待办：${s.currentTaskTitle}`:"";taskLabel.hidden=!s.currentTaskTitle;stats.textContent=`今日完成 ${t.todayCount} 个 · 近 7 天 ${t.weekCount} 个\n${s.focus} 分钟专注 / ${s.rest} 分钟短休息 · 每 4 个完成番茄使用 ${s.long} 分钟长休息。`;start.textContent=s.running?"暂停":"开始";document.title=s.running?`${fmt(s.remaining)} · ${s.mode} · Toolbox`:"Toolbox";};
    ...
    const next=(completed=false)=>{settings();if(s.mode==="专注"){if(completed){s.cycles++;s.history.unshift({endedAt:Date.now(),minutes:s.focus,taskId:s.currentTaskId||null,taskTitle:s.currentTaskTitle||""});s.history=s.history.slice(0,500);}s.mode=completed&&s.cycles%4===0?"长休息":"休息";s.remaining=(s.mode==="长休息"?s.long:s.rest)*60;}else{s.mode="专注";s.remaining=s.focus*60;}s.running=false;s.endAt=null;if(completed)sound();save();draw();};
    start.onclick=()=>{settings();if(ctx.taskId){s.currentTaskId=ctx.taskId;s.currentTaskTitle=ctx.task?.title||s.currentTaskTitle||"";}if(s.running){sync();s.running=false;s.endAt=null;}else{s.running=true;s.endAt=Date.now()+s.remaining*1000;}save();draw();};
  ...
  registerTool({ id:"focuslog", icon:"◎", name:"深度工作计时", category:"专注", desc:"针对一件待办计时，并保留任务级专注记录。", keywords:"深度工作 focus 计时 专注 记录 趋势 待办" }, (root,ctx={}) => {
    ...
    start.onclick=()=>{if(active)return;active={startedAt:Date.now(),task:task.value.trim()||"未命名专注",taskId:ctx.taskId||null,taskTitle:ctx.task?.title||""};persist();tick();ensureTimer();};
    finish.onclick=()=>{if(!active)return;const endedAt=Date.now(),elapsed=endedAt-active.startedAt;sessions.unshift({task:active.task,note:$("#fNote",root).value.trim(),taskId:active.taskId||null,taskTitle:active.taskTitle||"",startedAt:active.startedAt,endedAt,durationMs:elapsed,duration:fmt(elapsed),date:new Date(endedAt).toLocaleString()});sessions=sessions.slice(0,300);active=null;$("#fNote",root).value="";persist();ensureTimer();tick();show();};
