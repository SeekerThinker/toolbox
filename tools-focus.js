(() => {
  ...
  registerTool({ id:"pomodoro", icon:"◴", name:"番茄钟", category:"专注", desc:"自定义专注 / 休息循环并保留进度。", keywords:"番茄钟 pomodoro 专注 倒计时 休息" }, root => {
    const defaults = { focus:25, rest:5, long:15, cycles:0, mode:"专注", running:false, remaining:1500, endAt:null, history:[] };
    ...
    const trend=()=>{
      const keys=recentDayKeys(7), today=keys.at(-1);
      const valid=s.history.filter(x=>x&&x.endedAt&&keys.includes(localDayKey(x.endedAt)));
      const todayCount=valid.filter(x=>localDayKey(x.endedAt)===today).length;
      return { todayCount, weekCount:valid.length };
    };
    const draw=()=>{sync();const t=trend();time.textContent=fmt(s.remaining);mode.textContent=`${s.mode} · 已完成 ${s.cycles} 个番茄`;stats.textContent=`今日完成 ${t.todayCount} 个 · 近 7 天 ${t.weekCount} 个\n${s.focus} 分钟专注 / ${s.rest} 分钟短休息 · 每 4 个完成番茄使用 ${s.long} 分钟长休息。`;start.textContent=s.running?"暂停":"开始";document.title=s.running?`${fmt(s.remaining)} · ${s.mode} · Toolbox`:"Toolbox";};
  ...
  registerTool({ id:"focuslog", icon:"◎", name:"深度工作计时", category:"专注", desc:"为一件任务计时，并保留本地专注记录。", keywords:"深度工作 focus 计时 专注 记录 趋势" }, root => {
    ...
    const sessionMs=x=>Number(x.durationMs)||durationTextToMs(x.duration);
    const sessionTime=x=>Number(x.endedAt)||Date.parse(x.date)||0;
    const persist=()=>storage.set("tools.focuslog",{sessions,active});
    const show=()=>{
      const keys=recentDayKeys(7), totals=Object.fromEntries(keys.map(k=>[k,0]));
      let weekCount=0,weekMs=0,todayMs=0;
      for(const x of sessions){const key=localDayKey(sessionTime(x));if(!keys.includes(key))continue;const ms=sessionMs(x);totals[key]+=ms;weekCount++;weekMs+=ms;if(key===keys.at(-1))todayMs+=ms;}
      const daily=keys.map(k=>`${k.slice(5)} ${Math.round(totals[k]/60000)}m`).join(" · ");
      summary.textContent=`今日 ${compactDuration(todayMs)} · 近 7 天 ${compactDuration(weekMs)} · ${weekCount} 次${weekCount?` · 平均 ${compactDuration(weekMs/weekCount)}`:""}\n${daily}`;
      list.innerHTML=sessions.length?sessions.slice(0,12).map(x=>`<div class="session-item"><span><strong>${escapeHtml(x.task||"未命名专注")}</strong><br>${escapeHtml(x.note||"")}</span><span>${escapeHtml(x.duration||fmt(sessionMs(x)))}<br>${escapeHtml(x.date||new Date(sessionTime(x)).toLocaleString())}</span></div>`).join(""):"<span class='help-text'>还没有专注记录。</span>";
    };
    ...
    finish.onclick=()=>{if(!active)return;const endedAt=Date.now(),elapsed=endedAt-active.startedAt;sessions.unshift({task:active.task,note:$("#fNote",root).value.trim(),startedAt:active.startedAt,endedAt,durationMs:elapsed,duration:fmt(elapsed),date:new Date(endedAt).toLocaleString()});sessions=sessions.slice(0,100);active=null;$("#fNote",root).value="";persist();ensureTimer();tick();show();};
