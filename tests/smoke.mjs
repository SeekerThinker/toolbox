...
const runtimeFiles=["core.js","tools-focus.js","tools-thinking.js","tasks.js","horizons.js","calibration.js"];
const dormantFiles=["cloud-public.js","cloud-config.js","sync-policy.js","sync.js","account-lifecycle.js","cloud-diagnostics.js"];
const allFiles=[...runtimeFiles,...dormantFiles];
const index=fs.readFileSync("index.html","utf8");
for(const file of allFiles){
  if(!fs.existsSync(file))throw new Error(`missing ${file}`);
}
for(const file of runtimeFiles){
  if(!index.includes(`./${file}`))throw new Error(`guest runtime does not load ${file}`);
}
for(const file of dormantFiles){
  if(index.includes(`./${file}`))throw new Error(`guest runtime must not load dormant account file ${file}`);
}
for(const css of ["horizons.css","calibration.css","readability.css"]){if(!fs.existsSync(css)||!index.includes(`./${css}`))throw new Error(`missing guest style ${css}`);}
if(index.includes('./cloud.css'))throw new Error("guest runtime must not load dormant cloud styles");
...
if(index.includes('id="accountBtn"'))throw new Error("guest release should not render an account entry at all");
...
if(!horizons.includes("目标打卡")||!horizons.includes("关联任务")||!horizons.includes("targetPerWeek"))throw new Error("goal definition or task linking is incomplete");
if(horizons.includes("insightsView")||horizons.includes('data-horizon-tab="insights"'))throw new Error("planning must not duplicate calibration insights");
...
if(/data-cal-goal-save|data-cal-goal-target/.test(calibration))throw new Error("calibration should observe goal cadence, not edit it");

if(!cloudPublic.includes("publishableKey")||!cloudPublic.includes("emailOtp"))throw new Error("reserved email auth deployment config is incomplete");
if(/wechatProvider|phoneOtp|sms/i.test(cloudPublic))throw new Error("reserved auth config must stay email-only");
...
if(!sync.includes("syncPolicy.decide")||!sync.includes("syncPolicy.ACTIONS"))throw new Error("reserved sync must use the shared conflict policy");
for(const token of ["flowType: \"pkce\"","toolbox:data-changed","lastSyncedHash","lastSyncedRevision","SYNC_CONFLICT","开始同步","本机和云端"]){
  if(!sync.includes(token))throw new Error(`reserved sync layer missing token: ${token}`);
}
for(const token of ["当前：直接使用","不登录也能完整使用","signInWithOtp","verifyOtp","邮箱登录"]){
  if(!sync.includes(token))throw new Error(`reserved guest-first email auth missing token: ${token}`);
}
if(/微信|wechat|使用 GitHub 登录|signInWithOAuth|type:\s*"sms"/i.test(sync))throw new Error("reserved consumer auth must remain guest plus email only");
if(!sync.includes("enabled:false")||!sync.includes("不会先上传本机数据"))throw new Error("future cloud upload must remain explicit after login");
...
console.log(`ok: ${ids.length} methods, local-only guest runtime, dormant email auth, calibration and backend contracts intact`);
