...
for (const file of ["manifest.webmanifest", "pwa.js", "sw.js", "pwa-icon-192.svg", "pwa-icon-512.svg"]) {
...
if (!index.includes('src="./pwa.js"')) throw new Error("PWA bootstrap is not loaded");
...
if (!sw.includes("toolbox-shell-v1") || !sw.includes("request.mode === \"navigate\"")) throw new Error("offline app shell is incomplete");
...
console.log("ok: installable PWA shell, offline cache and safe cloud-config refresh enabled");
